...
在使用`bash_command`参数直接调用Bash脚本时，需要在脚本名称后添加空格。这是因为Airflow尝试将Jinja模板应用于它，这将会失败。
...
使用[`PythonOperator`](31.md)执行Python callables。
...
使用`op_args`和`op_kwargs`参数将额外参数传递给Python可调用对象。
...
在此示例中，参数值从Airflow变量中提取。此外，`default_args`字典用于将公共参数传递给单个DAG中的所有操作器。
...
如果你想要使用服务账号来运行或部署一个操作器，但得到了一个403禁止的错误，这意味着你的服务账号没有正确的Cloud IAM权限。
...
如果存在具有相同名称的实例，则不会执行任何操作，并且操作器将成功执行。
