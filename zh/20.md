...
请参阅[使用操作器](zh/9.md)了解如何使用Airflow操作器。
...
许多钩子都有一个默认的`conn_id`，使用该挂钩的操作器不需要提供显式连接ID。 例如，[`PostgresHook`](zh/31.md)的默认`conn_id`是`postgres_default` 。
...
请注意，XCom与[变量](zh/20.md)类似，但专门用于任务间通信而非全局设置。
...
Airflow充分利用了[Jinja Templating](http://jinja.pocoo.org/docs/dev/)的强大功能，这可以成为与宏结合使用的强大工具（参见[宏](zh/31.md)部分）。
